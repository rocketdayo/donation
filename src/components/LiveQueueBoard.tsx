...
export const LiveQueueBoard: React.FC<LiveQueueBoardProps> = ({
  ...
  const handleCallTicket = (ticket: TicketRecord) => {
    ...
    setRecentNotification(`整理券 #${ticket.ticketNumber} ${ticket.name}様に呼出通知を送信しました`);
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-slate-200/80 flex items-center justify-between gap-2">
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 text-xs transition"
                    title="呼出音・アナウンス再送"
                  >
